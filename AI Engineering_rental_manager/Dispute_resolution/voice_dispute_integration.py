"""
voice_dispute_integration.py
-------------------------------
Combined pipeline: finds complaints with a voice recording attached
but no AI suggestion yet, transcribes the audio (Whisper), classifies
the transcript (our dispute model), and writes a combined result back
to complaints/update.php's ai_suggestion field.
"""
...
dispute_model = joblib.load("dispute_model.pkl")
...
def get_voice_complaints_needing_suggestion(session):
    ...
    return [
        ...
        if c.get("voice_file") and not c.get("ai_suggestion")
    ]


def download_voice_file(session, filename):
    audio_url = f"{BASE_URL}/backend/uploads/voice/{filename}"
    resp = session.get(audio_url)
    if resp.status_code != 200:
        return None
    local_path = "temp_voice_complaint.audio"
...
def classify_and_suggest(transcript, agreement_terms):
    complaint_vec = vectorizer.transform([transcript])
    predicted_category = dispute_model.predict(complaint_vec)[0]
    probabilities = dispute_model.predict_proba(complaint_vec)[0]
    ...
    combined = (
        ...
        f"[{predicted_category}] {resolution_text}"
    )
...
def main():
    ...
    complaints = get_voice_complaints_needing_suggestion(session)
    print(f"\nFound {len(complaints)} voice complaint(s) needing processing.\n")

    for complaint in complaints:
        complaint_id = complaint["id"]
        voice_filename = complaint["voice_file"]
        agreement_id = complaint["agreement_id"]

        print(f"Complaint #{complaint_id} (voice file: {voice_filename}):")

        local_path = download_voice_file(session, voice_filename)
        if not local_path:
            print("  -> Could not download voice file, skipping.")
            continue
        ...
        suggestion, confidence = classify_and_suggest(transcript, agreement_terms)
        print(f"  Suggestion (confidence {confidence:.2f}) generated.")
